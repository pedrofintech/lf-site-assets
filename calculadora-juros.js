...
// (numeros + texto) e so depois do paint seguinte constroi a tabela e o
// grafico. Os calculos sao os da versao 56a2874, sem alteracoes - mudou apenas
// a ordem em que o DOM e atualizado.
...
var lfJurosRenderToken = 0;
function lfJurosAfterPaint(fn) {
  var token = ++lfJurosRenderToken;
  requestAnimationFrame(function () {
    setTimeout(function () {
      if (token !== lfJurosRenderToken) return;
      fn();
    }, 0);
  });
}
...
document.getElementById("calcular").addEventListener("click", function (e) {
  ...
  for (let period = 1; period <= periodYears * periodsPerYear; period++) {
    ...
    if (period % periodsPerYear === 0) {
      ...
      annualData.push({
        ...
        "Juros Acumulados": accumulatedInterest,
  ...
  // ---------------------------------------------------------------------
  // Fase 2 (depois do paint seguinte): tabela e grafico. E aqui que esta o
  // trabalho pesado (innerHTML da tabela + Chart.js), por isso sai do frame
  // que o INP mede.
  // ---------------------------------------------------------------------
  lfJurosAfterPaint(function () {
